...
numerical_dx_gradient=np.gradient(x[:,0])/np.gradient(tspan)
numerical_dx_diff=np.diff(x[:,0]) / np.diff(tspan)

py.figure(1)
py.plot(tspan,dx[:,0])
py.plot(tspan,numerical_dx_gradient)
py.plot(tspan[0:-1],numerical_dx_diff)

err_diff=np.abs(dx[0:-1,0]-numerical_dx_diff)
err_gradient=np.abs(dx[:,0]-numerical_dx_gradient)

py.figure(2)
py.plot(tspan,err_gradient)
py.plot(tspan[0:-1],err_diff)
py.legend(['np.gradient','np.diff'])
py.xlabel('Time-step')
py.ylabel('absolute relative error ')
